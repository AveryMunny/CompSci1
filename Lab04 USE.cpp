/* Program id: CSLab04.cpp
 ...
 should ask for the length and width of the rectangle and then display the rectangleís
 area. Use the following formula: area = length * width
 If the user enters 3 the program should ask for the length of the triangleís base and its
 ...
 */
 ...
 5. Display the respective area value depending on input.
 ...
 3. Calculate the area of a Triangle
    set area to .5 multiplied by the base multipled by the height

 4. Get various Area entries
    For the area of a circle, ask the user to input the radius of their circle. (Store as radius)
    For the area of a rectangle, ask the user to input the length of the rectangle, then ask
    the user to input the width of the rectangle. (Store each of these values as length, width respectively)
    For the area of a triangle, ask the user to input the base, along with the height of the triangle. (Store as base, height, respecitively)
    Calculate the areas by using the inputted values after they are given, and store the chosen area (shape) under the variable area.
...
float calculateRectangle()
{
    ...
    bool loopFlag = true;               // the control for validation
    string msg("The length or the width was invalid.");     // invalid message
    do
    {
...
float calculateTriangle()
{
    float base = 0;                    // base of the triangle
    float height = 0;                  // height of the triangle
    bool loopFlag = true;              // control for validation
    string msg("The base or the height was invalid.");      // invalid message
    do
    {
        cout << "Enter the triangle's base: ";
        cin >> base;
        loopFlag = validateFloatInput(base, 0, 999999999.99, msg);
        cout << "Enter the triangle's height: ";
        cin >> height;
        loopFlag = validateFloatInput(height, 0, 999999999.99, msg);
    } while (loopFlag);
